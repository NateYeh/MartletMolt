...
以下問題經評估後決定**暫不處理**，若下次審查時複雜度未提升則跳過：

| 檔案 | 函數 | 複雜度 | 排除原因 | 最後評估 |
|------|------|--------|----------|----------|
| `tools/test_doc_generator.py` | `test_generator` | 13 | 測試腳本，大量 assert 為正常行為，拆分會降低可讀性 | 2026-02-22 |
| `system_a/martlet_molt/cli.py` | `_display_history` | 11 | 32 行小函數，邏輯清晰，三元運算子多為正常格式化需求 | 2026-02-22 |
| `system_b/martlet_molt/cli.py` | `_display_history` | 11 | 同上（兩邊為同步副本） | 2026-02-22 |
...
**注意**：若複雜度**上升**超過 2 以上，應重新評估是否需要處理
---
