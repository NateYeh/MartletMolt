...
|:--- |:--- |:--- |:--- |
| `backend/system_x/martlet_molt/core/` | **系統大腦** | 修改 Agent 決策邏輯、Session 儲存、環境配置讀取。 | ✅ 可修改 |
| `backend/system_x/martlet_molt/tools/` | **Agent 工具箱** | **功能核心**。當你想賦予 Agent 新能力（如對接新 API、新檔案處理等）。 | ✅ 可修改 |
| `backend/system_x/martlet_molt/providers/` | **LLM 接口層** | 切換模型供應商或調整模型呼叫參數（如溫度、Max Tokens）。 | ✅ 可修改 |
| `backend/system_x/martlet_molt/gateway/` | **API 門戶** | 修改 REST API 端點、WebSocket 傳輸邏輯。 | ✅ 可修改 |
| `orchestrator/` | **守護程序 (Guardian)** | 涉及 A/B 切換邏輯、系統同步、部署自動化時。 | ❌ **限人手動** |
| `frontend/web-lite-v2/` | **介面層** | 修改聊天 UI、前端元件、Jinja2 模板或靜態資源。 | ✅ 可修改 |
| `tools/` | **開發輔助工具** | 需要執行代碼質量 Review、自動生成 API 文件等非運行時功能。 | ✅ 可修改 |
| `scripts/` | **後勤腳本** | 進行數據遷移、YAML 修復、同步開發環境配置。 | ✅ 可修改 |
