# TASK-012: 會話重命名功能支援 (待啟動 📄)

**建立日期**: 2025-02-22  
**優先級**: 中  
**狀態**: 📋 待開發

---

## 📋 任務概述

目前 `SessionManager` 雖然可以儲存會話，但缺乏一個明確的「標題 (Title)」欄位與對應的修改介面。本任務旨在後端實作會話重命名功能，以便用戶能更直觀地管理多個對話。

---

## 🎯 目錄與目標

- [ ] 在 `Session` 模型中增加 `title` 欄位（預設為 "新對話" 或訊息首句）。
- [ ] 實作 `PATCH /api/sessions/{session_id}` 端點。
- [ ] 更新 `SessionManager` 的儲存邏輯，支援原地更新會話元數據。
- [ ] 確保修改後，前端重新整理或讀取列表能看到新標題。

---

## 🛠️ 技術實作路徑

### 1. 模型更新
修改 `martlet_molt/core/session.py` 中的 `Session` 類別，確保包含標題屬性。

### 2. 管理器邏輯
在 `SessionManager` 增加 `update_session_title(session_id, title)` 方法。由於目前已切換為 SQLite 存儲，更新標題只需執行單條 SQL UPDATE 語句，效率極高。

### 3. API 實作
在 `martlet_molt/gateway/routes.py` 增加對應的隨機路由：
```python
@router.patch("/sessions/{session_id}")
async def rename_session(session_id: str, request: RenameRequest):
    ...
```

---

## 📁 修改路徑清單

- `backend/system_a/martlet_molt/core/session.py`
- `backend/system_a/martlet_molt/gateway/routes.py`

---

## ✅ 驗證方式

1. 使用 `curl` 或 Postman 對特定 `session_id` 發送 PATCH 請求。
2. 呼叫 `GET /api/sessions` 檢查回傳的列表中標題是否已變更。
3. 重啟後台服務，確認標題持久化成功。
4. 執行 `ruff check --fix` 與 `pyright`。
