...
@pytest.fixture
def ollama_provider():
    return OllamaProvider(api_key="fake-key", base_url="https://fake-ollama.com", model="test-model")


def test_convert_messages_basic(ollama_provider):
    """測試基本訊息格式轉換"""
    messages = [Message(role="system", content="sys content"), Message(role="user", content="user content"), Message(role="assistant", content="ai content")]
...
def test_convert_messages_with_tools(ollama_provider):
    """測試包含工具調用與工具回傳結果的轉換"""
    messages = [
        Message(role="assistant", content="Use tool", tool_calls=[{"id": "c1", "type": "function", "function": {"name": "test"}}]),
        Message(role="tool", content="tool result", name="test", tool_call_id="c1"),
    ]
...
def test_register_and_get_tools(ollama_provider):
    """測試工具註冊與定義輸出"""
    tool = ToolDefinition(name="test_tool", description="test description", parameters={"type": "object", "properties": {"cmd": {"type": "string"}}})
...
async def test_ollama_chat_success(ollama_provider):
    ...
    mock_response.json.return_value = {"message": {"role": "assistant", "content": "Hello human"}}
...
async def test_ollama_chat_with_tools(ollama_provider):
    ...
    mock_response.json.return_value = {
        "message": {"role": "assistant", "content": "Running command...", "tool_calls": [{"id": "call_99", "function": {"name": "shell", "arguments": {"command": "ls -la"}}}]}
    }
...
def test_get_available_models(ollama_provider):
    """測試模型清單取得"""
    mock_response = MagicMock()
    mock_response.json.return_value = {"models": [{"name": "glm-5:latest"}, {"name": "llama3:latest"}]}
